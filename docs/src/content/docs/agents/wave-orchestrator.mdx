...
8. On the first wave dispatch only, adds `Work:progress` to the feature issue to signal that execution is underway.
...
11. Removes `Work:progress` and adds `Work:done` to the feature issue to record that all waves have completed.
12. Creates the final feature PR (if it doesn't exist) with `Closes #task` for every task and `Closes #feature`. Comments that the feature is ready for review.
