...
| `max_turns` | The agent hit its turn limit — **partial work preserved** (the report names the branch) | `/fix` resumes from the preserved branch; consider `turns:<n>` for more headroom |
...
| `max_turns` | `execute` re-run | **Yes** — resumes the preserved `WIP:` branch instead of cutting a new one | No — the same branch is reused, so the eventual PR isn't a duplicate | Yes, once it completes and its PR merges |
| `max_turns` | `/fix` | Yes — always resumed the preserved branch | No — reuses the open PR if one already exists | Yes |
...
<Aside type="caution">
Before this table's `max_turns` row was true, re-issuing `/execute` on a task whose only trace was a preserved `WIP:` branch (no PR yet) silently cut a fresh branch from the base and orphaned the partial work — `/fix` was the only safe resume path. The Developer now resumes that branch either way, but `/fix` remains the documented remedy because it also carries the failure context (the last 10 comments) into the retry.
</Aside>
...
<Aside type="tip">
Comment *what went wrong* before running `/fix` — the agent reads recent comments as context.
</Aside>
