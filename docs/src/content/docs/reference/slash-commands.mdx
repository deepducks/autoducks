...
| `engineer` | `tactics` |
| `execute` | `run`, `work` |
| `fix` / `revert` / `close` / `review` / `rework` / `defer` | *(none)* |
...
- **Only `architect`, `engineer`, `execute`, and `review` are chainable.** Utility verbs (`fix`/`revert`/`close`/`rework`/`defer`) never carry or continue a chain — they always run standalone, even if you write `#auto:fix`, `#auto:revert`, `#auto:close`, `#auto:rework`, or `#auto:defer`.
