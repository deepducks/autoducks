...
3. Adds the `Spec:draft` label to the issue to indicate that design work is in progress.
...
8. Removes `Spec:draft` and adds `Spec:plan` to the issue to record that the design phase is complete.
...
- **`Draft` label removed**
- **`Spec:draft` removed; `Spec:plan` added** — design phase marked complete
