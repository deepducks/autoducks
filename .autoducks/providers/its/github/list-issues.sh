...
# (defaults to 50 when unset) so a single call can never pull more than the
# run is budgeted for; when omitted, the configured max is used outright.
...
its::list_issues() {
  ...
  local max_issues
  max_issues="$(jq -r '.product.max_issues_per_run // 50' "$AUTODUCKS_ROOT/autoducks.json" 2>/dev/null)"
  [[ -z "$max_issues" || "$max_issues" == "null" ]] && max_issues=50
