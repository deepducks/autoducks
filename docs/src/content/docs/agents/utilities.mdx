...
**Scripts:** `.autoducks/agents/rework/{pre,post}.sh` · **Trigger:** `/rework` on the feature/bug issue or directly on its pull request
...
6. `post.sh` parses that task with the same parser the Engineer uses, appends it as a new task on the feature issue (a new wave, or promoting a single-task issue into a two-wave plan), and reverts the PR to draft.
