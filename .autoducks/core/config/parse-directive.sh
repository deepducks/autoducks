...
#   command          — canonical verb: architect, engineer, execute, fix, revert, close, review, rework, defer
...
normalize_verb() {
  ...
  if [[ -f "$_CONFIG_FILE" ]] && command -v jq &>/dev/null; then
    local _agent _alias
    for _agent in architect engineer execute fix revert close review rework defer; do
...
is_canonical_verb() {
  case "$1" in
    architect|engineer|execute|fix|revert|close|review|rework|defer) return 0 ;;
    *) return 1 ;;
  esac
}
