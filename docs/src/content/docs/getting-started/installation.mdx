...
6. **Issue types (Feature, Task)** — checks org-level issue types. Workflows run without them, but typed issue relationships won't appear in the issue tracker UI. Routing is driven by the `Feature` label (which the tactical and design agents apply automatically); the native type is a visual enhancement for org repos.

7. **Sub-issues API** — GitHub's native sub-issue relationship. Available
   on every GitHub-hosted repository as of October 2024. The Tactical agent
