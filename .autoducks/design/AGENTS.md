...
**Verification and the tool clamp.** The no-ceiling rule above holds only for definitions that are merged, reviewed repo content. Discovery scans the live checkout, which on a PR surface is `refs/pull/N/head`, so each definition is compared against `AUTODUCKS_BASE_REF` (`origin/<default_branch>`) with `git diff`. The descriptor records the outcome in `verified`: `base` when the file matches, `unverified` when it was added or edited, `unchecked` when no base ref is configured (a local `setup.sh` run). An `unverified` definition falls back to `unverified_tools` in `.autoducks/agents/agent/defaults.json` — a strictly read-only set with no `Write`, `Edit`, `WebFetch` or `WebSearch` — regardless of what its frontmatter or config asks for. Because the config is the higher-precedence source, the `tools` grant is read from the **base ref's** `autoducks.json`, not the checked-out one; otherwise a PR touching only that file could escalate an otherwise unmodified definition. Everything else the config supplies is not a privilege and is read from the live file. Because clamping the grant does not make the run trustworthy — the definition body is still unreviewed content, it still becomes the prompt, and whatever the agent reads ends up in a public comment — an `unverified` definition is **refused** by default, before the LLM step. Setting `custom_agents.allow_unverified: true` opts back in to trying an agent from the pull request that introduces it; the read-only clamp still applies in that mode, so the opt-in widens who may run, never what they may reach.
