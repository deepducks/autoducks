...
- **Only `architect`, `engineer`, `execute`, and `review` are chainable.** Utility verbs (`fix`/`revert`/`close`) never carry or continue a chain — they always run standalone, even if you write `#auto:fix`, `#auto:revert`, or `#auto:close`.
