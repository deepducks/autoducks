...
**Gitlink reconciliation (`metarepo::reconcile_gitlinks`).** Fast-forwards an open parent PR's gitlinks to each child's current default-branch tip. It covers the two staleness sources delivery cannot: an async auto-merge that could not report its resulting SHA, and a **sibling parent PR merging**, which moves the base's gitlink out from under every other open parent PR (this is what flipped `meta#108` to `CONFLICTING` four minutes after `meta#97` merged). It runs from the delivery poller for one PR, and from the `repin-siblings` job of the same workflow — triggered on `pull_request: closed` + `merged == true` — for every *other* open parent PR. `metarepo::pin_relation` gates it via the compare API: only a **`behind`** pin (an ancestor of the tip) is moved; `ahead` means the delivery has not merged yet and is left alone, `diverged` is reported for a human. Because a reconcile commit changes the PR head, the poller mirrors its conclusion onto the new head SHA too, so the required check stays satisfied even when the push credential fires no `synchronize` event. A gitlink moved by a **direct push** to the default branch (no PR) is the deliberate residual gap — such PRs reconcile on their next delivery-check run rather than immediately.
