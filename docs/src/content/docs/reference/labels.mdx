...
| Label | Applied by | Meaning |
|-------|-----------|---------|
| `Feature` | Architect | Routing + classification; pipeline branch `feature/…` |
| `Bug` | Architect | Same pipeline as features; pipeline branch `fix/…` |
| `Intake:Feature` | Product Owner | Provisional triage hint proposed at intake — **not** route-critical, does not affect branch naming; the Architect confirms or overrides it when classifying and strips the label |
| `Intake:Bug` | Product Owner | Same as `Intake:Feature`, proposing `Bug` instead |
...
Labels are the route-critical signal on every repository kind; the native issue *types* (Feature/Task/Bug) are a best-effort visual enhancement on organization repos. `Intake:*` is the one exception — it is a non-authoritative hint, never consulted for routing.
