...
| `mcpServers` | | MCP server names this plugin registers; each must exist in `claude/mcp.json`. |
| `claudeHooks` | | Claude Code hook lifecycle events this plugin wires up; each must exist in `claude/hooks.json`. |
...
- **One step per contributor, in an aggregator action.** For every hook point at least one plugin (or a local contribution, see below) targets, the compiler generates `.github/actions/autoducks/<point>/action.yml` — a composite action whose steps `uses:` each contributor's `hooks/<point>/action.yml` in turn, forwarding the same [environment contract](/guides/customization#environment-contract) documented for hand-authored hooks.
- **Duplicate plugin names are rejected.** Two `plugins[]` entries with the same `name` hard-fail before anything is compiled.
- **Duplicate MCP server names hard-fail.** If two plugins targeting the same agent both register an MCP server with the same name, the compiler refuses rather than silently letting the later one win.
- **Conflicting `env` values hard-fail; identical values merge silently.** If two plugins' `claude/settings.patch.json` set `env.SOME_KEY` to *different* values for the same agent, the compiler refuses. The same key set to the same value by multiple plugins is fine.
...
- **Everything else in `settings.patch.json` merges as a JSON merge-patch**, layered on top of the repository's base `.autoducks/providers/llm/claude/settings.json` (or `{}` if that file doesn't exist), in declared plugin order.
