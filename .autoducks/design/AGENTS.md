...
**Verification and the tool clamp.** The no-ceiling rule above holds only for definitions that are merged, reviewed repo content. Discovery scans the live checkout, which on a PR surface is `refs/pull/N/head`, so each definition is compared, with `git diff`, against the **merge-base** of `AUTODUCKS_BASE_REF` (`origin/<default_branch>`) and `HEAD` — the merge-base rather than the branch tip, so a definition is not marked changed merely because the default branch moved after the branch was cut. The descriptor records the outcome in `verified`: `base` when the file matches, `unverified` when it was added or edited, `unchecked` when no base ref is configured (a local `setup.sh` run). An `unverified` definition falls back to `unverified_tools` in `.autoducks/agents/agent/defaults.json` — a strictly read-only set with no `Write`, `Edit`, `WebFetch` or `WebSearch` — regardless of what its frontmatter or config asks for. Every privilege-bearing config key is read from that same merge-base, never from the checked-out tree: the `tools` grant (the higher-precedence source, so a PR touching only `autoducks.json` could otherwise escalate an unmodified definition), `custom_agents.enabled` (the repo owner's kill switch), `custom_agents.roots[]`, and `allow_unverified` itself. Everything else the config supplies is not a privilege and is read from the live file. Because clamping the grant does not make the run trustworthy — the definition body is still unreviewed content, it still becomes the prompt, and whatever the agent reads ends up in a public comment — an `unverified` definition is **refused** by default, before the LLM step. Setting `custom_agents.allow_unverified: true` **on the default branch** opts back in to trying an agent from the pull request that introduces it. The flag is read from `AUTODUCKS_BASE_REF`, never from the checked-out tree, so it cannot be switched on by the same pull request it would permit — it is a repo-owner decision made ahead of time. The read-only clamp still applies in that mode, so the opt-in widens who may run, never what they may reach.
