...
```
/<trigger> [model:<model>] [effort:<level>] [turns:<n>] [#auto:<agent>+<agent>...]
```
...
| `engineer` | `tactics` |
| `execute` | `run`, `work` |
| `fix` / `revert` / `close` / `review` / `triage` / `merge` | *(none)* |
...
- **Only `architect`, `engineer`, `execute`, and `review` are chainable.** Utility verbs (`fix`/`revert`/`close`/`triage`/`merge`) never carry or continue a chain — they always run standalone, even if you write `#auto:fix`, `#auto:revert`, `#auto:close`, `#auto:triage`, or `#auto:merge`.
