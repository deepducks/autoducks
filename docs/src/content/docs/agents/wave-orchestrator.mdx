...
8. On the first wave dispatch for the feature, adds the `Work:progress` label to the feature issue to signal execution is now active.
9. Dispatches execution agents in parallel for each undone task, skipping already-done or already-dispatched tasks (duplicate dispatch prevention).
10. Posts a summary comment listing dispatched and skipped tasks.

**If all waves are done:**
11. Creates the final feature PR (if it doesn't exist) with `Closes #task` for every task and `Closes #feature`. Comments that the feature is ready for review.
12. Removes the `Work:progress` label from the feature issue and adds `Work:done` to mark orchestration complete.

**If blocked** (not all previous waves done):
13. Comments that orchestration is waiting for dependencies.
