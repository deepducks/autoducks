...
3. **[LLM]** Reads `/tmp/triage-inbox.md`/`.json` and writes `/tmp/triage-decisions.json` **only**: proposed `priorities` (`Critical`/`High`/`Medium`/`Low`, each with a rationale) and, sweep-only, proposed `duplicates` groupings (`canonical` + `duplicates` + `confidence` + rationale). Strictly read-only — no `git`, `gh`, or `Edit`.
...
6. `dry_run: true` (workflow-dispatch only) describes the proposal without applying anything.
