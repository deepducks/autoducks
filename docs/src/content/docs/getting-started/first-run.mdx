...
1. `/quack architect` — review the design in the issue body; edit anything you want to steer. Re-running the Architect **revises** rather than rewrites, and never touches the plan zone.

2. `/quack engineer` — review the tasks and waves. Re-running enters [revision mode](/agents/engineer#revision-mode) (existing tasks preserved by number). If the design was insufficient, the Engineer posts blocking questions instead of guessing — answer them and re-run.

3. `/quack execute` — the Maestro takes it from here, exactly as in Path A.

</Steps>

<Aside type="tip">
You can also chain stages with explicit stops: `/quack architect #auto:engineer` designs and plans, then waits for you before any code is written. See [Chaining & overrides](/guides/chaining-and-overrides).
</Aside>
...
- `/quack revert` — undo the planning: closes task issues, restores your original issue body, deletes bot comments.
- `/quack close` — full teardown: closes tasks and PRs, deletes branches, closes the issue.
