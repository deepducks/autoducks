...
autoducks runs four pipeline agents — **Architect → Engineer → Maestro → Developer** — plus three utilities. You drive them with one command surface, and each agent guards its own entry: if a prerequisite is missing, the agent **dispatches it automatically** and re-queues itself behind it (the *Definition-of-Ready cascade*).
...
| You comment `/quack execute` on… | Who answers |
|---|---|
...
So `/quack execute` on a fresh issue runs the whole pipeline: Architect → Engineer → Maestro → Developers. See the [lifecycle walkthrough](/guides/pipeline-lifecycle) for what that looks like comment by comment.

## Utility commands

| Command | Type | What it does |
|---------|------|--------------|
| [`/quack review`](/agents/reviewer) | LLM | Reviews a PR against its design and acceptance criteria — read-only, never merges |
| [`/quack fix`](/agents/utilities#quack-fix) | LLM | Repairs a failed task run, resuming preserved partial work |
| [`/quack revert`](/agents/utilities#quack-revert) | Bash | Undoes planning, restores the human-authored issue |
| [`/quack close`](/agents/utilities#quack-close) | Bash | Full cleanup: closes issues, PRs, and deletes branches |
...
| Needs LLM | Pure bash |
|-----------|-----------|
| Architect, Engineer, Developer, Reviewer, Fix | Maestro, Revert, Close |
...
The bot never edits *your* comment — reactions stay on it, and `/quack revert` can safely delete all bot content while preserving everything human-authored.
