...
5. Strips the six progress labels (`Spec:draft`, `Spec:plan`, `Tactics:crafting`, `Tactics:ready`, `Work:progress`, `Work:done`) from the feature issue and each closed task so the pipeline lineage is fully cleared.
6. Restores the original issue body from edit history (last non-bot edit).
7. Deletes all bot comments on the feature issue.
8. Reacts with 👍.
...
5. Strips the six progress labels (`Spec:draft`, `Spec:plan`, `Tactics:crafting`, `Tactics:ready`, `Work:progress`, `Work:done`) from the feature issue and every task before closing them.
6. Closes the feature issue with a cleanup summary.
7. Reacts with 👍.
