...
4. Removes `Ready`, `draft`, and all six progress labels (`Spec:draft`, `Spec:plan`, `Tactics:crafting`, `Tactics:ready`, `Work:progress`, `Work:done`) from the feature issue.
...
5. Strips all progress labels (`Spec:draft`, `Spec:plan`, `Tactics:crafting`, `Tactics:ready`, `Work:progress`, `Work:done`) along with `Ready` and `draft` from the feature issue as part of cleanup.
