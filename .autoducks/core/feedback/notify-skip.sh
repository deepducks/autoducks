...
notify_skip() {
  ...
  local body="ℹ️ **Auto-review skipped.** This PR modifies autoducks agent workflow files, so \`claude-code-action\` refuses to run the auto-triggered review against repository secrets (an anti-tampering safeguard). This is expected, not a failure.
