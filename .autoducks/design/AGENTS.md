...
**Delivery (children-first, parent last).** At feature completion Maestro runs `git::submodule_deliver` per affected child **before** the parent final PR: an **unprotected** child has its default branch fast-forwarded and its feature branch deleted (SHA now reachable on `main`); a **protected** child gets a PR carrying the skip-marker, auto-merged by default. The human merges the parent last. Protected delivery uses a **merge commit**, never squash/rebase: the parent gitlink pins the *feature-branch* SHA, so the merge method must keep that commit reachable from the child's default branch — a squash rewrites the SHA and would orphan the pinned gitlink once the branch is deleted (HANDOFF gotcha #7). Squash is a last-resort fallback (child disallows merge commits) that keeps the feature branch and warns.
