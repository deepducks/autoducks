...
autoducks ships two supported customization surfaces: **hooks**, which run your own steps around each agent, and **prompt overrides**, which extend or replace an agent's instructions. Both live outside the files autoducks manages, so they survive `install.sh` updates.
...
There are 16 hook points: one pre and one post hook for each of the 8 agent workflows.
...
|----------|------------|-------|
| `AUTODUCKS_AGENT` | all 16 hooks | the agent name, e.g. `developer` |
| `AUTODUCKS_STAGE` | all 16 hooks | `pre` or `post` |
| `ISSUE_NUM` | all 16 hooks | the triggering issue/feature number |
| `REPO` | all 16 hooks | `owner/repo` |
| `COMMENT_ID` | all 16 hooks | the triggering comment's ID, or `0` |
| `RUN_ID` | all 16 hooks | the GitHub Actions run ID |
| `COMMENTER` | all 16 hooks | the GitHub login that triggered the run |
| `GH_TOKEN` | all 16 hooks | `AUTODUCKS_PAT` if set, else the default `GITHUB_TOKEN` |
| `BASE_BRANCH` | developer only | the branch the Developer is building on |
| `IS_PR` | reviewer only | `true` if the trigger was a PR event |
...
Only the 5 LLM-backed agents support prompt overrides: **architect**, **engineer**, **developer**, **reviewer**, and **fix**. Maestro, close, and revert are bash-only orchestration agents with no prompt to customize.
