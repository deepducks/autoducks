...
**Delivery (children-first, parent last).** At feature completion Maestro runs `git::submodule_deliver` per affected child **before** the parent final PR; the human merges the parent last. The parent gitlink pins the child *feature-branch* SHA (`Y`), so delivery must keep `Y` reachable from the child's default branch, or re-point the pin. `git::submodule_deliver` honors `AUTODUCKS_MERGE_METHOD` (`merge`|`squash`|`rebase`|`auto`; `auto` prefers a merge commit for children) and returns the SHA the parent should pin:
...
- **Protected, or `squash`/`rebase` policy** → open a child PR carrying the skip-marker and merge it with the configured method. A **merge commit** keeps `Y` reachable → pin unchanged. A **squash/rebase rewrites the SHA** to a new commit `S` on the default branch: `Y` is abandoned, so Maestro **re-pins** the parent gitlink to `S` (via `metarepo::repin_gitlinks`, a `git update-index --cacheinfo` bump committed onto the feature branch) and only then deletes the retained child branch. This is how the autoducks squash policy is supported without orphaning the pin (HANDOFF gotcha #7).
